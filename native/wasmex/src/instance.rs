...
use rustler::{
    ...
    Encoder, Env, Error, MapIterator, NifMap, NifResult, OwnedEnv, ResourceArc, Term,
    TermType,
};
use std::ops::Deref;
use std::sync::Mutex;
...
pub fn call_exported_function(
    ...
    TOKIO_RUNTIME.spawn(async move {
        ...
        thread_env.run(|env| {
            ...
            let result_term = result.load(env).decode::<Term>().unwrap_or(atoms::error().encode(env));
            
            // GenServer.call from tuple is {pid, ref}
            // LocalPid in Rustler can handle both local and remote PIDs (despite the name)
            let (caller_pid, ref_term) = from_tuple.decode::<(LocalPid, Term)>()
...
fn execute_function_async(
    ...
    let result = thread_env.run(|env: Env| {
        let given_params = match function_params.load(env).decode::<Vec<Term>>() {
            Ok(vec) => vec,
            Err(_) => return env.error_tuple("could not load 'function params'").encode(env),
        };
        ...
        let function = match function_result {
            Some(func) => func,
            None => {
                return env.error_tuple(&format!("exported function `{function_name}` not found")).encode(env)
            }
        };
        ...
        match call_result {
            ...
            Err(err) => {
                ...
                if let Ok(trap) = err.downcast::<Trap>() {
                    return env.error_tuple(&format!("Error during function excecution ({trap}): {reason}")).encode(env);
                } else {
                    return env.error_tuple(&format!("Error during function excecution: {reason}")).encode(env);
                }
            }
        };
        ...
        for value in results.iter().cloned() {
            return_values.push(match value {
                ...
                Val::FuncRef(_) => {
                    return env.error_tuple("unable_to_return_func_ref_type").encode(env)
                }
                Val::ExternRef(_) => {
                    return env.error_tuple("unable_to_return_extern_ref_type").encode(env)
                }
                Val::AnyRef(_) => {
